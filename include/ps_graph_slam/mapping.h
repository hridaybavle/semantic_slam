...
public:
  ...
  void setoptimizedKeyframes(
      std::vector<ps_graph_slam::KeyFrame::Ptr> keyframes_optimized) {
  ...
  void generateMap() {
    while (1) {
      ...
      for (int i = 0; i < keyframes.size(); ++i) {
        ...
        // pcl::PointCloud<pcl::PointXYZRGB>::Ptr out_cloud (new
        // pcl::PointCloud<pcl::PointXYZRGB>());
        Eigen::MatrixXf pose = ps_graph_slam::matrix2vector(
            keyframes[i]->robot_pose.matrix().cast<float>());

        Eigen::Matrix4f transformation_mat;
        sem_tool_obj_.transformMapPointsToWorld(pose, transformation_mat,
                                                cam_angle_);
        ...
        current_map_cloud.keyframe_pose =
            keyframes[i]->robot_pose.matrix().cast<float>();
    ...
    while (1) {
      ...
      for (int i = 0; i < map_cloud_vec_.size(); ++i) {
        ...
        if (opt_keyframe.size() > 0) {
          // pcl::PointCloud<pcl::PointXYZRGB>::Ptr out_cloud (new
          // pcl::PointCloud<pcl::PointXYZRGB>());
          Eigen::VectorXf pose_opt = ps_graph_slam::matrix2vector(
              opt_keyframe[i]->node->estimate().matrix().cast<float>());
          Eigen::VectorXf pose_old =
              ps_graph_slam::matrix2vector(map_cloud_vec_[i].keyframe_pose);

          // Eigen::Matrix3f r_old =
          // map_cloud_vec_[i].keyframe_pose.block<3,3>(0,0); Eigen::Matrix3f
          // r_opt =
          // opt_keyframe[i]->node->estimate().matrix().cast<float>().block<3,3>(0,0);
          // double yaw_drift = semantic_tools::R2ypr(r_opt).x() -
          // semantic_tools::R2ypr(r_old).x();

          Eigen::VectorXf pose_diff = pose_opt - pose_old;

          // Eigen::Matrix4f pose_diff =
          // opt_keyframe[i]->node->estimate().matrix().inverse().cast<float>() *
          // map_cloud_vec_[i].keyframe_pose;
          ...
          for (int j = 0; j < map_cloud_vec_[i].out_cloud->points.size(); ++j) {
            ...
            map_cloud_vec_[i].out_cloud->points[j].x =
                old_points(0) + pose_diff(0);
            map_cloud_vec_[i].out_cloud->points[j].y =
                old_points(1) + pose_diff(1);
            map_cloud_vec_[i].out_cloud->points[j].z =
                old_points(2) + pose_diff(2);
            map_cloud_vec_[i].keyframe_pose =
                opt_keyframe[i]->node->estimate().matrix().cast<float>();
          }
        }
  ...
  processPointCloud(sensor_msgs::PointCloud2 point_cloud_msg) {
  ...
  std::vector<map_cloud> getOutputMap() { return map_cloud_vec_; }
};
